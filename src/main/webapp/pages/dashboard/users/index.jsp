...
<div class="container-fluid px-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 mt-4">
        ...
        <div>
            ...
            <a href="${pageContext.request.contextPath}/dashboard/user-roles"
                        ...
                        <div>
                            ...
                            <h2 class="mb-0 fw-bold">
                                ...
                                <c:forEach var="user" items="${users}">
                                    <c:if test="${user.status == true}"><c:set var="activeUsers"
                                                                               value="${activeUsers + 1}"/></c:if>
                                </c:forEach>
                                ${activeUsers}
                            </h2>
                        </div>
                            ...
                            <h2 class="mb-0 fw-bold">
                                ...
                                <c:forEach var="user" items="${users}">
                                    <c:forEach var="role" items="${user.roles}">
                                        <c:if test="${role == 'ADMIN'}"><c:set var="adminUsers"
                                                                               value="${adminUsers + 1}"/></c:if>
                                    </c:forEach>
                                </c:forEach>
                                ${adminUsers}
                            </h2>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h2 class="mb-0 fw-bold">
                                ...
                                <c:forEach var="user" items="${users}">
                                    <c:if test="${user.status == false}"><c:set var="inactiveUsers"
                                                                                value="${inactiveUsers + 1}"/></c:if>
                                </c:forEach>
                                ${inactiveUsers}
                            </h2>
                        </div>
                ...
                ${role}<c:if test="${!loop.last}">, </c:if>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center">
                                    <a href="${pageContext.request.contextPath}/dashboard/users/${user.id}"
                                       class="btn btn-sm btn-outline-info action-btn" data-bs-toggle="tooltip"
                                       data-bs-title="Xem chi tiết">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="${pageContext.request.contextPath}/dashboard/users/${user.id}/edit"
                                       class="btn btn-sm btn-outline-primary action-btn" data-bs-toggle="tooltip"
                                       data-bs-title="Chỉnh sửa">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="${pageContext.request.contextPath}/dashboard/user-roles/edit?userId=${user.id}"
                                       class="btn btn-sm btn-outline-warning action-btn" data-bs-toggle="tooltip"
                                       data-bs-title="Phân quyền">
                                        <i class="fas fa-user-tag"></i>
                                    </a>
                                    <form action="${pageContext.request.contextPath}/dashboard/users/${user.id}"
